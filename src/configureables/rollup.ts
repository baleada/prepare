...
export class Rollup {
  ...
  input (file: string | string[]) {
    ...
    return typeof file === 'string' ? this : this.multi()
  }
  ...
  multi (options?: any) {
    return this.plugin(multi(options))
  }
...
function createVirtual (): Virtual {
  function virtual (config: Record<any, any> | ((api: VirtualApi) => Record<any, any>)) {
    return this.plugin(virtual(ensureVirtualConfig(config)))
  }

  function index (path, createFilesToIndexOptions: Record<any, any> = {}) {
    return (this as Rollup).virtual(({ testable }) => ({
  ...
  function iconComponents ({ icons }: { icons: Icon[] }) {
    return (this as Rollup).virtual(({ testable }) => ({
      test: api => 
        icons.some(({ componentName }) => testable.idEndsWith(`src/components/${componentName}.vue`).test(api))
        &&
        testable.queryIsEmpty().test(api),
...
type Virtual = {
  (config: Record<any, any> | ((api: VirtualApi) => Record<any, any>)): Rollup,
  index: (...args: any[]) => Rollup
  routes: (...args: any[]) => Rollup
...
type VirtualApi = { testable: Testable }

function ensureVirtualConfig (rawConfig: Record<any, any> | ((api: VirtualApi) => Record<any, any>)) { // TODO: Type virtual config
  return typeof rawConfig === 'function'
    ? rawConfig({ testable: new Testable() })
    : rawConfig
}
